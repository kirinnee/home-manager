...
tags=$(curl -s "${url}" | grep -oP '(?<=/zed-industries/zed/releases/tag/)[vV]?([0-9]+\.){2}[0-9]+' | sed 's/^v//' | sort -V)

# Get the latest tag
latest_tag=$(echo "${tags}" | tail -n 1)

# generate the url
dl_url="https://github.com/zed-industries/zed/releases/download/v${latest_tag}/Zed-arm64.dmg"
