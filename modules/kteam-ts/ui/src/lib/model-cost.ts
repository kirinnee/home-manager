// Transcript-evidenced API cost estimation.  This module intentionally owns
// the complete price registry: callers must never guess a model price.
...
export interface PricingTokenFields {
  /** Schema-v6 durable event evidence. Only explicit false is safe: true means
   * wrapper billing changed mid-session; missing means a legacy/unknown row. */
...
function entryFor(model: string): PricingEntry | undefined {
  return PRICING_REGISTRY.find(entry => entry.aliases.includes(model));
}
...
export function estimateModelCost(billing: BillingKind, row: PricingTokenFields): ModelCost {
  ...
  const entry = entryFor(row.pricingModel);
  if (!entry) return { kind: 'unknown', reason: 'unknown_pricing_model' };
  if (!withinValidityWindow(entry, row.createdAt)) {
    return { kind: 'unknown', reason: 'pricing_outside_validity_window', pricingKey: entry.pricingKey };
  }
