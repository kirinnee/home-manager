...
import {
  NeedsYouTrigger,
  actorLabel,
  needsYouTriggerLabel,
  needsYouUnreachableCopy,
  waitingAgeCopy,
} from './NeedsYouPanel';

describe('needs-you copy and age', () => {
  test('trigger always names the surface and carries a count when nonzero', () => {
    expect(needsYouTriggerLabel(0)).toBe('Needs you');
    expect(needsYouTriggerLabel(4)).toBe('Needs you (4)');
  });
  ...
  test('unreachable state refuses to pretend the list is empty', () => {
    expect(needsYouUnreachableCopy()).toMatch(/out of date/i);
  });
});

test('trigger exposes a labelled 44px target and visible badge', () => {
  const html = renderToStaticMarkup(
    <NeedsYouTrigger id="needs" count={7} expanded={false} onClick={() => undefined} />,
  );
  expect(html).toContain('aria-label="Needs you (7)"');
