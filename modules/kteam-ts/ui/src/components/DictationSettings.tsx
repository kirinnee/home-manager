...
export const ENHANCEMENT_TOGGLE_EXPLANATION =
  'Optional correction runs once after local transcription. Choose instant on-device word correction or hosted Groq cleanup below. They cannot send a message for you.';
...
export const GROQ_ENHANCEMENT_EXPLANATION =
  'Groq runs only after dictation stops; it is never in the live-text path. It is instructed to preserve your wording while correcting recognition errors, spelling, capitalization and punctuation. The daemon reads GROQ_API_KEY from ~/.secrets and sends Groq the raw transcript, your dictionary and bounded context. The key is never stored in this browser. A timeout or provider error keeps your raw words and shows the real reason instead of silently falling back.';

export const DICTATION_SAFETY_NOTE =
  'Microphone audio stays in this browser. Stop settles the newest bounded local window and runs one enhancement pass, then inserts the result once at your current caret. Nothing is ever sent for you.';

export const DICTATION_DISABLED_EXPLANATION =
  'Off means off: the microphone cannot start, model files are not fetched, no decode runs, and loaded ONNX sessions are released to give back about 1 GB of memory. The prepared offline copy stays on disk, so turning dictation back on works without a reload or another download.';
    ...
    <div className="flex flex-col gap-5">
      ...
      {settings.enabled ? (
        <>
          ...
          >
            ...
            {browserWeightsMissing && browserModel && (
              <div className="flex flex-col gap-2">
                <p className="text-meta leading-base text-warn">
                  Your box has not downloaded the browser model yet, so this device has nothing to fetch. That is a
                  one-time {formatBytes(browserModel.costs.downloadBytes)} download onto the box, shared by every
                  device.
                </p>
            ...
            <p className="text-meta leading-base text-muted">
              {formatBytes(localEngineTotalBytes())} in total: the speech model plus the runtime that executes it,
              served from <code className="text-faint">{STT_MODEL_BASE}</code> on your own box.
            </p>
            {localState === 'ready' && (
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-meta leading-base text-muted">
              One per line. Add alternatives after an “=”, separated by commas — <code>kteam = kteem, katim</code>.
        ...
        <p className="rounded-control border border-border bg-surface-2 p-3 text-meta leading-base text-faint">
          Dictation is disabled. Turn it on above whenever you want the microphone and speech model available again.
        </p>
      )}
