...
const PRECACHE_OUT = join(SW_DIR, 'precache.gen.ts');
...
async function main(): Promise<void> {
  ...
  if (existsSync(SW_ENTRY)) {
    ...
    run(['bun', 'build', 'sw/sw.ts', '--outfile', outfile, '--target', 'browser'], 'worker bundle');
    console.log(`postbuild-pwa: bundled worker → ${workerName(release)}`);
  } else {
