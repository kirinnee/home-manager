...
providers. Stored credentials use read-only usage endpoints; inference-scoped external
Anthropic tokens use the smallest possible message request because they cannot access the
profile-scoped usage endpoint:

| Provider    | Accounts                           | Source                                                                                           |
| ----------- | ---------------------------------- | ------------------------------------------------------------------------------------------------ |
| `anthropic` | Claude Max/Pro (stored OAuth)      | `GET api.anthropic.com/api/oauth/usage` (token from the Keychain/credentials file; no inference) |
| `anthropic` | Claude Max/Pro (declared external) | `POST api.anthropic.com/v1/messages`, Haiku with `max_tokens: 1`; quota response headers         |
...
The two Anthropic sources have deliberately separate conversions. Stored OAuth JSON
returns `utilization` as `0..100` percentage points and stays unscaled. Declared external
tokens lack the JSON endpoint's `user:profile` scope, so kfleet reads
`anthropic-ratelimit-unified-{5h,7d}-{status,utilization,reset}` from the minimal inference
response. Header utilization is a `0..1` fraction and is multiplied by 100; reset is a Unix
epoch converted to milliseconds. HTTP 429 is still a successful quota probe when those
headers are valid. The request is deduped by exact token, so each external account consumes
at most one `max_tokens: 1` Haiku probe per configured usage cycle. The schema default is
60 seconds; this repository's `kfleet/config.yaml` overrides it to 300 seconds.

For a provider token managed outside `kfleet login`, declare the source on the
agent itself:
...
- name: loge1
  ...
  env:
    CLAUDE_CODE_OAUTH_TOKEN: $LOGE_CLAUDE_1_TOKEN # wrapper runtime env only
```

This is an explicit login exclusion: `kfleet login` never scans, clones,
refreshes, or interactively replaces it. Usage resolves the declared key from
the inherited environment when present, otherwise by safely sourcing the fixed,
generated `~/.secrets` file itself. That makes `kfleet serve` deterministic under a stripped
systemd/launchd environment. It never infers a usage credential from
`CLAUDE_CODE_OAUTH_TOKEN`, and it never falls back to a stored credential from a
different account. `~/.secrets` is the shell file generated by this
repository's `load-secrets` activation.
...
Declared `secrets-file` credentials remain outside that login flow: rotate them
with `kloge pull`, run `hms` to regenerate `~/.secrets`, then re-check usage.
...
still needs an interactive `claude`/`codex login`. Background usage probing in `serve` is
**ON by default**; tune it via `config.yaml` (including the interval that bounds external
Anthropic inference probes):

```yaml
usage:
  enabled: true # default true — external Anthropic credentials use max_tokens:1 probes
...
`credential` is agent-only (never profile-merged). The supported external form
is `{ source: secrets-file, key: NAME }`; it always resolves the generated
`~/.secrets` file and is copied unchanged to every resolved variant.
