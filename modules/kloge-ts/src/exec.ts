...
// Mirrors modules/khost-ts/src/exec.ts so kloge reads like its siblings.
...
export interface RunOpts {
  ...
  input?: string;
  /** Inherit stdio (interactive: docker logs -f, ssh). */
  interactive?: boolean;
...
/** Run, or die with the captured stderr if it fails. Returns stdout. */
export async function must(cmd: string[], opts: RunOpts = {}): Promise<string> {
  const r = await run(cmd, opts);
  if (r.code !== 0) {
    die(`command failed (${r.code}): ${cmd.join(' ')}\n${r.stderr.trim() || r.stdout.trim()}`);
  }
  return r.stdout;
}
...
export async function dockerCompose(args: string[], opts: RunOpts = {}): Promise<RunResult> {
  ...
  die("neither 'docker compose' nor 'docker-compose' found (is OrbStack/Docker running?)");
}
