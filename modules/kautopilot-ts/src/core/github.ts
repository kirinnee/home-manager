...
function parseJson<T>(stdout: string, label: string): T {
	try {
		...
		throw new Error(
			`gh ${label}: invalid JSON output: ${stdout.slice(0, 200)}`,
		);
	}
}
...
export async function ghPrChecks(
	prNumber: number,
	cwd?: string,
): Promise<CheckStatus[]> {
...
export async function ghPrView(
	...
	const result = await gh(
		[
			...
			String(prNumber),
			"--json",
			"mergeable,mergeStateStatus,headRefName,state,url,reviews,createdAt",
		],
		cwd,
	);
...
export async function ghReviewThreads(
	prNumber: number,
	cwd?: string,
): Promise<PollThread[]> {
	const query = `
  ...
  `;
	...
	const data = parseJson<GhReviewThreadsResponse>(
		result.stdout,
		"review-threads",
	);
...
export async function ghReviews(
	prNumber: number,
	cwd?: string,
): Promise<GhReview[]> {
	const repoInfo = await ghRepoInfo(cwd);
	const result = await gh(
		[
			"api",
			`repos/${repoInfo.owner}/${repoInfo.repo}/pulls/${prNumber}/reviews`,
		],
		cwd,
	);
...
export async function ghReplyToThread(
	prNumber: number,
	commentId: number,
	body: string,
	cwd?: string,
...
export async function ghReplyToIssueComment(
	...
	const args = [
		"api",
		`repos/${repoInfo.owner}/${repoInfo.repo}/issues/${prNumber}/comments`,
		"-f",
		`body=${body}`,
	];
...
export async function ghResolveThread(
	threadId: string,
	cwd?: string,
): Promise<void> {
	const mutation = `
...
export async function ghClosePr(prNumber: number, cwd?: string): Promise<void> {
	const result = await gh(
		[
			...
			String(prNumber),
			"--comment",
			"Closed for rollover — creating fresh PR",
		],
		cwd,
	);
...
export async function ghListPrsForBranch(
	branch: string,
	cwd?: string,
...
export async function ghFetchMergePolicy(
	owner: string,
	repo: string,
	cwd?: string,
): Promise<MergePolicyInfo> {
	const query = `
  ...
  `;
	...
	return {
		...
		requiredApprovingReviewCount: Math.max(
			...rules.map((r) => r.requiredApprovingReviewCount),
			0,
		),
		requiresStatusChecks: rules.some((r) => r.requiresStatusChecks),
		requiredStatusCheckContexts: rules.flatMap(
			(r) => r.requiredStatusCheckContexts,
		),
...
export async function ghRunLogsFailed(
	runId: string,
	cwd?: string,
): Promise<string> {
