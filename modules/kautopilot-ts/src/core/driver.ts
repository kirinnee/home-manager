...
import type { DoneDescriptor, NextResult, StepContext, StepDef, StepDescriptor, StepPhase } from './descriptor';
import { appendEvent, readLog } from './log';
import { findRepo, type RepoEntry, readSessionMeta, type SessionMeta } from './session-meta';
 ...
 */
function isCursorEvent(meta: Record<string, unknown> | undefined): meta is Record<string, unknown> {
...
function planApprovedThisEpoch(log: ReturnType<typeof readLog>, epoch: number): boolean {
  return log.some(
    e =>
 ...
 */
function pendingStep(sessionId: string, repo: string | null, epoch: number): string | null {
  const log = readLog(sessionId);
  let pending: string | null = repo == null ? ENTRY_STEP : planApprovedThisEpoch(log, epoch) ? REPO_ENTRY : null;
...
function resolveRepo(meta: SessionMeta, def: StepDef, repoArg: string | null): RepoEntry | null {
...
function buildContext(meta: SessionMeta, config: Config, def: StepDef, repoArg: string | null): StepContext {
  return {
...
function maybeEmitPhaseMarker(sessionId: string, phase: StepPhase, version: number): void {
...
function doneResult(meta: SessionMeta, phase: StepPhase | 'done', reason: string): DoneDescriptor {
 ...
 */
export async function runNext(sessionId: string, config: Config, repoArg: string | null = null): Promise<NextResult> {
  ...
  for (let guard = 0; guard < 1000; guard++) {
    ...
    if (stepName === AWAIT_REPOS) {
      ...
      if (freshMetaPre.repos.length === 0 || notReady.length > 0) {
        const drive = notReady.map(r => `kautopilot next --repo ${r.repo}`).join('; ');
        return doneResult(
          meta,
          'execution',
