 ...
 */
export function writeKloopConfig(
  ...
  // Build YAML config matching kloop's format
  const yaml = [
 ...
 */
export async function devloopRun(kloopRunId: string, timeoutMs?: number): Promise<DevloopRunResult> {
  ...
  const timeout = timeoutMs ?? 600_000; // 10 minute default
