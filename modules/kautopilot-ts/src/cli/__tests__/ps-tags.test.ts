...
describe("createSession persists lpsm + tags; ps tag filtering end-to-end", () => {
	...
	it("createSession stores lpsm when provided and omits it otherwise", () => {
		const { createSession } =
			require("../../core/session-create") as typeof import("../../core/session-create");
		const { readSessionMeta } =
			require("../../core/session-meta") as typeof import("../../core/session-meta");
	...
	it("createSession stores free-form tags when non-empty and omits otherwise", () => {
		...
		expect(readSessionMeta(tagged.sessionId)?.tags).toEqual([
			"urgent",
			"spike",
		]);
	...
	it("matchesTags over persisted sessions filters lpsm + free-form tags", () => {
		...
		expect(matchesTags(carbonMeta?.lpsm, carbonMeta?.tags, ["nitrite"])).toBe(
			true,
		);
		...
		expect(
			matchesTags(neonMeta?.lpsm, neonMeta?.tags, ["nitrite", "neon"]),
		).toBe(true);
		expect(
			matchesTags(carbonMeta?.lpsm, carbonMeta?.tags, ["nitrite", "neon"]),
		).toBe(false);
		...
		expect(
			matchesTags(freeMeta?.lpsm, freeMeta?.tags, ["urgent", "nitrite"]),
		).toBe(true);
