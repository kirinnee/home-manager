...
export function createDiffCommand(): Command {
	return new Command("diff")
		...
		.argument(
			"<artifact>",
			"triage | spec | plan[:<repo>] | feedback | brainstorm",
		)
		...
		.action(
			async (
				artifact: string,
				opts: { from?: number; to?: number; session?: string },
			) => {
				try {
					...
					if (!KINDS.has(rawKind)) {
						logError(
							`Unknown artifact: ${rawKind}. Use triage|spec|plan[:repo]|feedback|brainstorm.`,
						);
						process.exit(1);
					}
					...
					if (kind === "plans" && !resolvedRepo) {
						...
						if (repos.length === 1) {
							...
							logError(
								`No plan revisions found for epoch ${meta.epoch}. Run plan writing first.`,
							);
							process.exit(1);
						} else {
							logError(
								`Multiple plan repos exist — specify one as \`plan:<repo>\`. Available: ${repos.join(", ")}.`,
							);
							process.exit(1);
						}
					}
