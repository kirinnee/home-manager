...
export function resolveSession(sessionIdArg?: string): ResolvedSession {
  ...
  if (!sessionId) {
    throw new Error('No session found. Pass --session <id> or run inside a session worktree.');
  }
  const meta = readSessionMeta(sessionId);
  if (!meta) throw new Error(`Session ${sessionId} has no session.json (not a host-driven session).`);
