 ...
 */
async function runHttpServer({ host, port }: { host: string; port: number }): Promise<never> {
  ...
  try {
    ...
    if (code === 'EADDRINUSE') {
      ...
      logError(`Failed to start web UI: ${err instanceof Error ? err.message : String(err)}`);
    }
    process.exit(1);
  }
