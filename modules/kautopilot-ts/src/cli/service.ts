...
// ============================================================================
// `kautopilot service <install|uninstall|status|restart>` — run
// `kautopilot serve` as a self-managed, always-on background service via the
// OS's native per-user service manager. No Nix / home-manager involvement: the
// binary writes and loads the unit itself, so it works anywhere the binary is
// installed.
//
//   macOS  -> launchd user agent   (~/Library/LaunchAgents/<LABEL>.plist)
//   Linux  -> systemd user service (~/.config/systemd/user/<UNIT>)
//
// It runs the native `serve` (not the dockerized `dash`): no Docker dependency.
// ============================================================================

const TOOL = "kautopilot";
const LABEL = "com.kirin.kautopilot-serve"; // launchd label (macOS)
const UNIT = "kautopilot-serve.service"; // systemd unit name (Linux)

// Absolute bun + entrypoint, captured at install time so the service needs no
// PATH and no Nix wrapper. Re-run `kautopilot service install` after a bun upgrade.
...
const sysQuote = (s: string): string =>
...
const macRestart = (): boolean =>
	sh(["launchctl", "kickstart", "-k", `gui/${uid()}/${LABEL}`]).ok;
const macRunning = (): boolean =>
	sh(["launchctl", "print", `gui/${uid()}/${LABEL}`]).ok;
const macInstalled = (): boolean => existsSync(plistPath());
const macPort = (): number | null =>
	readPort(plistPath(), /<string>--port<\/string>\s*<string>(\d+)<\/string>/);
...
function linuxInstall(port: number): void {
	...
	const unit = `[Unit]
Description=${TOOL} dashboard (serve)
...
`;
	...
	if (!sh(["loginctl", "enable-linger"]).ok) {
		logDim("note: enable-linger failed — service runs only while logged in");
	}
}
...
const linuxRestart = (): boolean =>
	sh(["systemctl", "--user", "restart", UNIT]).ok;
...
function report(b: Backend, port: number): void {
	...
	const line = `${TOOL} service: ${inst ? "installed" : "not installed"}, ${running ? "running" : "stopped"}`;
	if (inst && running) logOk(line);
	else if (inst) logError(line);
	else logInfo(line);
	if (inst) logInfo(`http://localhost:${port}`);
}
...
export function createServiceCommand(): Command {
	...
	svc
		...
		.action((opts: { port?: number }) => {
			...
			logOk(`${TOOL} service installed — http://localhost:${port}`);
			logInfo(`${TOOL} service status      check state`);
			logInfo(`${TOOL} service restart     reload after editing src/`);
			logInfo(`${TOOL} service uninstall   stop + remove`);
		});
