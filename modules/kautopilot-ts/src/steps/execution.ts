...
import { createBranch, detectDefaultBranch, getCurrentBranch, getGitRoot } from '../core/git';
...
import { authoringRepoName, latestPlanFiles, resolveActivePlans } from '../phases/shared';
...
function requireRepo(ctx: StepContext): RepoEntry {
  if (!ctx.repo) throw new Error('execution step requires a repo (ctx.repo is null)');
  return ctx.repo;
}
...
function requireWorktree(ctx: StepContext): string {
  ...
  if (!repo.worktree) throw new Error(`execution step requires repo "${repo.repo}" to have a worktree`);
...
function worktreeSpecDir(ctx: StepContext): string {
  return join(requireWorktree(ctx), 'spec', ticketId(ctx.meta), `v${ctx.version}`);
}
...
function readRepoState<T>(ctx: StepContext, key: string): T | undefined {
  ...
  const events = readLog(ctx.sessionId)
    .filter(e => e.event === 'context:updated' && e.repo === repo && e.version === ctx.version)
...
export function lastEventMetadata(
  ...
  const last = readLog(sessionId)
    .filter(e => e.event === event && (e.repo ?? e.metadata?.repo ?? null) === repo && (e.version ?? 0) === epoch)
    .at(-1);
 ...
 */
function provisionWorktree(repoPath: string, name: string, fallback: string): { worktree: string; branch: string } {
  ...
  run: async ctx => {
    ...
    if (worktree && existsSync(worktree)) {
      ...
      try {
        ...
        const planFolders = latestPlanFiles(ctx.sessionId, ctx.version, authoringRepoName(ctx.meta));
 ...
 */
function ensureFeatureBranch(worktree: string, branch: string | null, baseBranch: string): void {
  if (!branch) return;
  try {
    getGitRoot(worktree);
  ...
  run: async ctx => {
    ...
    if (planPath && existsSync(planPath)) {
      try {
        ...
        const specPath = writeKloopSpec(ctx.sessionId, planContent, `${planName}-spec.md`);
        const kloopRunId = devloopInit(requireWorktree(ctx), specPath);
        recordRepoState(ctx, { kloopRunId });
      } catch (err) {
        console.warn(`[setup_run] kloop init skipped: ${(err as Error).message}`);
      }
    }
...
const running: StepDef = {
  ...
  run: async ctx => {
    ...
    let status: 'completed' | 'max_iterations' | 'conflict' | 'crash' = 'completed';
    if (kloopRunId) {
      try {
        const result = await devloopRun(kloopRunId);
        status = result.status;
      } catch (err) {
        console.warn(`[running] kloop run failed to start: ${(err as Error).message}`);
        status = 'crash';
      }
    }
...
const runningSubagent: StepDef = {
  ...
  prepare: async ctx => {
    const planIndex = currentPlanIndex(ctx);
    const plans = repoPlanPaths(ctx);
    const planPath = plans[planIndex] ?? join(worktreePlansDir(ctx), `${planNameFor(planIndex)}.md`);
...
const resolve: StepDef = {
  ...
  prepare: async ctx => {
    ...
    const body = getAgentPrompt('phase2', 'resolve', vars as Record<string, string>);
    return {
      ...
      contract: {
        ...
        completionMetadataSchema: {
          rewriteDecision: 'refine_local|patch_downstream|regenerate_remaining|revisit_spec|retry',
        },
      },
    } satisfies PreparedStep;
  },
...
type AmendStrategy = 'refine_local' | 'patch_downstream' | 'regenerate_remaining';
...
const amendPlans: StepDef = {
  ...
  prepare: async ctx => {
    ...
    const decision = (readRepoState<string>(ctx, 'rewriteDecision') ?? 'refine_local') as AmendStrategy;

    // Completed/incomplete plan lists, derived from this repo's plan files.
    const completed = planPaths.slice(0, planIndex).map((_, i) => `- ${planNameFor(i)} (completed)`);
    const incomplete = planPaths.slice(planIndex).map((_, i) => `- ${planNameFor(planIndex + i)}`);
    ...
    const vars: Record<string, string | null> = {
      ...
      completed_plans_list: completed.length > 0 ? completed.join('\n') : '(none)',
      incomplete_plans_list: incomplete.length > 0 ? incomplete.join('\n') : '(none)',
    };

    const header = substitute(strategyHeader(decision), vars);
    const body = getAgentPrompt('phase2', 'amend_plans', vars as Record<string, string>);
    return {
      prompt: `${header}\n\n${SHARED_APPROVAL_GATE}\n\n${body}`,
      vars,
      contract: {
...
const commit: StepDef = {
  ...
  prepare: async ctx => {
    const planIndex = currentPlanIndex(ctx);
    const planPaths = repoPlanPaths(ctx);
    const planPath = planPaths[planIndex] ?? join(worktreePlansDir(ctx), `${planNameFor(planIndex)}.md`);
