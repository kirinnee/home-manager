...
const DOC_KINDS = new Set<ArtifactKind>(['triage', 'spec', 'feedback', 'brainstorm']);
...
function planRevisions(id: string, repo: string, epoch: number): RevisionInfo[] {
...
export function getDoc(id: string, kind: 'ticket' | ArtifactKind, version?: number): DocView | null {
  const meta = readSessionMeta(id);
  ...
  const epoch = meta.epoch;
  const versions = kind === 'brainstorm' ? brainstormRevisions(id) : listRevisionsWithEpoch(id, kind, { epoch });
  if (versions.length === 0) return { markdown: '', version: null, versions: [] };
...
export function getPlan(id: string, repo: string, version?: number): DocView | null {
  const meta = readSessionMeta(id);
  if (meta == null) return null;
  const epoch = meta.epoch;
