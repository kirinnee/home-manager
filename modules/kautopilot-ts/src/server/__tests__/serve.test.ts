...
describe('serve', () => {
  ...
  function writeSession(id: string): void {
    ...
    writeFileSync(join(dir, 'epoch', '2', 'spec', 'v1.md'), '# Spec\n\nFirst version.\n');
    writeFileSync(join(dir, 'epoch', '2', 'spec', 'v2.md'), '# Spec\n\nSecond version.\n');
    // Brainstorm precedes any epoch: two versions, no brainstorm:approved
    // events, so they must surface with epoch: null.
    writeFileSync(join(dir, 'brainstorm', 'v1.md'), '# Brainstorm\n\nFirst idea.\n');
    writeFileSync(join(dir, 'brainstorm', 'v2.md'), '# Brainstorm\n\nSecond idea.\n');
  ...
  it('serves the API and the SPA shell against a temp HOME', async () => {
    ...
    const { handleRequest } = require('../routes') as typeof import('../routes');
    ...
    try {
      ...
      // /api/sessions/:id
      const detail = (await (await fetch(`${base}/api/sessions/${id}`)).json()) as any;
      ...
      expect(pageHtml).toContain('<meta name="viewport" content="width=device-width, initial-scale=1">');
  ...
  it('renders an empty session list for an empty HOME without crashing', async () => {
    ...
    try {
      const list = await (await fetch(`http://127.0.0.1:${server.port}/api/sessions`)).json();
      expect(list).toEqual([]);
    } finally {
      server.stop(true);
    }
