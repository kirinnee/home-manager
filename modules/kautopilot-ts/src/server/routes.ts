...
import { getDiff, getDoc, getPlan, getSessionDetail, isDocKind, listSessionSummaries, storeFingerprint } from './data';
...
function handleApi(parts: string[], url: URL): Response | null {
  ...
  if (section === 'doc') {
    ...
    const version = parts[5] === 'v' && parts[6] ? Number.parseInt(parts[6], 10) : undefined;
  ...
  if (section === 'diff') {
    const from = url.searchParams.has('from') ? Number.parseInt(url.searchParams.get('from') as string, 10) : undefined;
    const to = url.searchParams.has('to') ? Number.parseInt(url.searchParams.get('to') as string, 10) : undefined;
    if (parts[4] === 'plans') {
