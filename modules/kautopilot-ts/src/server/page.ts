...
const CLIENT_SCRIPT = [
	...
	"  return out;",
	"}",
	"function redlineMarkdown(fromMd, toMd) {",
	"  const diff = lcsDiff(tokenizeMd(fromMd), tokenizeMd(toMd));",
	"  const wrap = (t, s) => !s ? '' : t === 'ins' ? '<ins class=\"d-ins\">' + s + '</ins>' : t === 'del' ? '<del class=\"d-del\">' + s + '</del>' : s;",
	"  let out = '', buf = '', cur = 'eq';",
	"  for (const seg of diff) { if (seg.t !== cur) { out += wrap(cur, buf); buf = ''; cur = seg.t; } buf += seg.s; }",
	"  return out + wrap(cur, buf);",
	"}",
	...
	"  app.appendChild(head);",
	"  // Rendered markdown redline: prose with inline insertions/deletions.",
	"  await renderMarkdown(redlineMarkdown(d.fromMarkdown || '', d.toMarkdown || ''));",
	"}",
