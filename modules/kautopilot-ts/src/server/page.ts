...
const CLIENT_SCRIPT = [
	...
	"  // wrap tables so they scroll inside a rounded box on mobile",
	'  div.querySelectorAll("table").forEach((t) => {',
	"    if (t.parentElement && t.parentElement.classList.contains('table-wrap')) return;",
	'    const w = el("div", "table-wrap");',
	"    t.replaceWith(w); w.appendChild(t);",
	"  });",
	"  // mermaid: replace the <pre><code class=language-mermaid> with a card",
	...
	"  if (blocks.length) {",
	"    try { await mermaid.run({ nodes: blocks }); } catch (_e) { /* leave source visible */ }",
	"  }",
	"}",
	"// Render a unified diff with a faint line gutter and red/green rows.",
	"function renderDiff(text) {",
	'  const wrap = el("div", "diff-wrap");',
	'  for (const line of String(text).split("\\n")) {',
	"    let cls = 'ctx', g = ' ';",
	"    if (/^\\+\\+\\+|^---|^@@|^diff |^index /.test(line)) { cls = 'meta'; g = ' '; }",
	"    else if (line.startsWith('+')) { cls = 'add'; g = '+'; }",
	"    else if (line.startsWith('-')) { cls = 'del'; g = '-'; }",
	'    const row = el("div", "ln " + cls);',
	'    const gut = el("span", "gutter"); gut.textContent = g;',
	'    const code = el("span", "code"); code.textContent = line || " ";',
	"    row.appendChild(gut); row.appendChild(code); wrap.appendChild(row);",
	"  }",
	...
	"    : '/sessions/' + encodeURIComponent(id) + '/diff/' + kind;",
	"  showSkeleton(1);",
	"  const d = await api(apiPath);",
	'  app.innerHTML = "";',
	"  main.classList.add('prose-page');",
	...
	"  // Derive a v{from} → v{to} label from the diff header lines when present.",
	"  let vfrom = '', vto = '';",
	"  const mfrom = /---\\s+\\S*v(\\d+)/.exec(d.diff || '');",
	"  const mto = /\\+\\+\\+\\s+\\S*v(\\d+)/.exec(d.diff || '');",
	"  if (mfrom) vfrom = mfrom[1]; if (mto) vto = mto[1];",
	"  const vstr = (vfrom && vto) ? '<span class=\"vlabel\">v' + esc(vfrom) + ' → v' + esc(vto) + '</span>' : '';",
