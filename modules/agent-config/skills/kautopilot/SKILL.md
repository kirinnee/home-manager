...
loop:
  ...
  for plan in s.ready:
      ...
      drive kloop for that plan (init → run → poll); RESOLVE CONFLICTS yourself
...
4. Poll `kloop status <id> --json` until it's no longer `running` (`kloop logs -f <id>`
   to watch). Read `kloop describe <id>` once for a summary.
