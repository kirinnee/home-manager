...
- The AI agent fleet (per-account `claude-<name>` / `codex-<name>` wrappers + their settings/memory/skills/hooks) is managed by **`kfleet`**, NOT Nix. To change accounts: edit `~/.kfleet/config.yaml` (and the asset files under `~/.kfleet/`: `templates/`, `skills/`, `hooks/`, `CLAUDE.md`), then run `kfleet apply` (`kfleet apply --prune` to drop stale ones). Everything composes from `profiles`; a wrapper is generated per **agent × variant**, so define each model ONCE and let the `auto` variant clone the `auto-*` wrappers — do NOT hand-add `auto-*` agents, and do NOT hand-edit `~/.claude-<name>/` etc. (`apply` regenerates them). Run `kfleet doctor` to sanity-check; see `modules/kfleet-ts/README.md` for the config format. The canonical source for shared assets (this memory, skills, hooks, statusline) is `modules/agent-config/`; `kfleet init` seeds `~/.kfleet/` from there. Only `multi-gh`/`multi-gws` accounts are still Nix-managed (edit `home-template.nix` + `hms`).
