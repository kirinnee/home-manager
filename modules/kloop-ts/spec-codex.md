...
Wrapper packaging is handled by `kfleet` (generates the `codex-<name>` wrappers from `~/.kfleet/config.yaml`) — no wrapper changes in this repo.
...
Use wrappers generated by `kfleet`. These already produce account-specific wrapper binaries (e.g., `codex-kirin`, `codex-mm3`) that set environment variables for API keys and model defaults.

### Wrapper Responsibilities (managed by kfleet)
...
- `default.nix` — wrapper packaging is handled by `kfleet` (generates `codex-<name>` wrappers from `~/.kfleet/config.yaml`)
...
- [ ] Codex wrapper binaries (e.g., `codex-kirin`) are available via `kfleet` (`~/.kfleet/bin`)
