...
export async function installHandler(): Promise<void> {
  const homeDir = os.homedir();
  const skillDir = path.join(homeDir, '.claude', 'skills', 'kloop');
  const skillFile = path.join(skillDir, 'SKILL.md');

  try {
    await fs.mkdir(skillDir, { recursive: true });
    await fs.writeFile(skillFile, SKILL_CONTENT, 'utf-8');
    console.log(pc.green('kloop skill installed successfully!'));
    console.log(pc.dim(`  ${skillFile}`));
    console.log('');
    console.log('Use it in Claude Code with:');
    console.log(pc.bold('  /kloop'));
  } catch (err) {
