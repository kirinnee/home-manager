...
// ============================================================================
// `kloop service <install|uninstall|status|restart>` — run `kloop serve` as a
...
// and correct PID-based crash detection (the dash container can't see host PIDs).
// ============================================================================

const TOOL = 'kloop';
const LABEL = 'com.kirin.kloop-serve'; // launchd label (macOS)
const UNIT = 'kloop-serve.service'; // systemd unit name (Linux)

// Absolute bun + entrypoint, captured at install time so the service needs no
// PATH and no Nix wrapper. Re-run `kloop service install` after a bun upgrade.
...
function report(b: Backend, port: number): void {
  ...
  if (inst) console.log(pc.dim(`http://localhost:${port}`));
}
...
export function createServiceCommand(): Command {
  ...
  svc
    ...
    .action((opts: { port?: number }) => {
      ...
      console.log(pc.green(`${TOOL} service installed — http://localhost:${port}`));
