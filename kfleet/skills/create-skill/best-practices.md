...
The `description` field is **critical** for skill discovery. The agent reads descriptions to decide when to activate a skill.
...
The agent loads supporting files only when needed, saving context.
...
> **Claude-only — breaks portability.** `allowed-tools` is a Claude Code
> frontmatter key; other harnesses (Codex, Cursor, …) ignore it, so the
> restriction silently vanishes there. Only use it in deliberately
> Claude-only skills, and say so in the first line of the skill body.
...
1. **Trigger Test**: Ask the agent something matching your description
...
```bash
# Commit skill as a unit (<skills-dir> = wherever the skill lives,
# e.g. kfleet/skills/ + kfleet/skills-codex/ for fleet skills, or
# .claude/skills/ / .agents/skills/ per-project — see "Where skills live" in SKILL.md)
git add <skills-dir>/my-skill/
