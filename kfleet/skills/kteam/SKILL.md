...
- `claude-auto-loge1..6` are direct first-party accounts and use native aliases. The pooled `claude-auto-loge` proxy uses real model IDs and intentionally has no Fable mapping while the pool is blocked.
...
Each wrapper already carries its own default model (kfleet's `KTEAM_MODEL`: `opus` for standard Claude accounts, `fable` where Fable is allowed (`opus` → Opus 5.5, `fable` → Fable 5.1 on Anthropic accounts), `gpt-6-sol` for Codex — ChatGPT codex accounts serve real IDs only: `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6-*`, `gpt-5.5`; bare aliases like `sol` are rejected), so you normally omit the model. Override only when a task needs a specific one with `--model <alias|id>`, e.g. `kteam start --agent claude-auto-kirin --model sonnet --cwd "$PWD" "…"`. Leave it off to keep the account default.
