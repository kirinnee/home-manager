...
- **kteam model routing** (full table + handoff chain in the `/kteam` skill): chain = Fable main thread → planner (kteam Fable or GPT-6 Astra; GPT-6 Sol/Opus 5.5 OK for low-ambiguity plans; planners may spawn implementers) → implementer. Implementers: GPT-6 Astra @ ultra for the hardest long many-checkpoint workloads; Opus 5.5 / GPT-6 Sol for generic→mid-high (Sol is also the default reviewer); GLM-5.3 sparingly (mechanical/frontend); GPT-6 Luna/Haiku for trivial. GPT-5.6-terra/5.5 are previous-gen and implement only from a plan by Fable/Astra/Sol/Opus. Product-facing: NEVER MiniMax M3 or DeepSeek V4. mm3 + GLM-5.3 = mass-chore tier only (divide-and-conquer, 1 file = 1 agent). Big context needs ≥ Opus/terra — and sol or Fable only when implementing against it.
