...
Throughout these examples, `<skills-dir>` is wherever the skill lives — e.g.
`kfleet/skills/<name>/` + `kfleet/skills-codex/<name>/` for fleet skills, or
`.claude/skills/` (Claude) / `.agents/skills/` (Codex) for per-project skills.
See "Where skills live" in [SKILL.md](SKILL.md).
...
```text
<skills-dir>/create-k8s-deployment/
├── SKILL.md
├── reference.md
├── examples.md
└── templates/
...
A skill that only reads files. Note this uses `allowed-tools`, which only
Claude Code enforces — that makes the skill Claude-only, so its body discloses
that in the first line (per the portability rule in [SKILL.md](SKILL.md)).
...
Claude-only skill: relies on `allowed-tools`, which other harnesses ignore.
...
Use Glob to find files matching patterns. This skill cannot run Bash — the
`allowed-tools` list above restricts it to Read, Grep, and Glob.
...
```bash
bun <skills-dir>/data-processor/scripts/validate.ts input.csv
```

### Step 2: Transform Data

```bash
bun <skills-dir>/data-processor/scripts/transform.ts input.csv output.csv
```
````
