---
name: create-skill
description: Create a new agent skill in the open SKILL.md standard (works with
  Claude Code AND Codex). Use when asked to create a skill, make a skill, or
  build a reusable agent workflow/automation. Generates a SKILL.md plus optional
  supporting files.
---
...
`crc-*`/`claude-auto-*`) if you want it to run under codex too. If a skill is
inherently single-harness, say so in the first line of the body.
...
### Step 2: Choose the destination, then create the directory + SKILL.md

First decide where the skill lives (details in "Where skills live" below):

- **Fleet skill** (shared across the kfleet agents): create BOTH twins —
  `kfleet/skills/<skill-name>/` AND `kfleet/skills-codex/<skill-name>/` in the
  home-manager repo — then run `kfleet apply` to deploy them. Keep supporting
  files identical across the twins; only SKILL.md frontmatter may differ
  (codex: `name` + `description` (+ `metadata`) only).
- **Per-project skill**: `.claude/skills/<skill-name>/` (Claude) or
  `.agents/skills/<skill-name>/` (Codex) inside the project repo.
...
See [templates/SKILL-TEMPLATE.md](templates/SKILL-TEMPLATE.md) for a fill-in template.
...
4. Fleet skills: both twins exist (`kfleet/skills/` + `kfleet/skills-codex/`) and
   `kfleet apply` has been run so the deployed homes pick them up.
