...
loop:
  ...
  if d.execution == "deferred":   # deferred writer step — do NOT run d.prompt
      follow relay.md VERBATIM: drive turns with `kautopilot relay` (parked via a
      delegated blocking subagent on Codex), present each envelope, complete
      after approval. The reviewer fan-out, visuals, spikes, and `revise` are the
      WRITER's job in this mode — never yours. (The writer is always a Claude
      account, regardless of this session's harness — the binary drives it.)
      Deferred mode needs **NO context reset** — the writer session holds the
      heavy context, so the main session stays lean; just continue the loop
      after the relay approval flow completes.
...
all — send a desktop notification** in the SAME turn you present the ask. On
Codex/macOS use:

```
osascript -e 'display notification "spec v3 ready — 2 questions" with title "kautopilot" sound name "Glass"'
```

One line, lead with what they must do. This includes (not exhaustive — the rule
is "any human input or human-relevant milestone"):
...
6. **Bot signature** on PR replies: `"By Codex kautopilot 🤖"`.
