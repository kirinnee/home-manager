...
## Two names, and kteam composes the bracketed one for you (Claude only)
...
- **The Claude-side session name** — you do **not** pass this. For a **Claude** session,
  kteam automatically names Claude's own session **`[Teammate] Task`** (e.g.
  `[Hayden] Fix Transcript`) at launch, so it shows up that way in claude.ai/code and the
...
**Codex differs:** Codex has **no launch-time display-name flag**, so kteam composes **no**
`[Teammate] Task` harness-side name for a Codex session. There, `--name` is still the plain
task title (the TASK column) and `--teammate` is still the callsign — there is just no
separate Claude-side name to speak of. Pass `--name`/`--teammate` the same way; only the
composed second name is absent.

You still pick the teammate name **before** you start (kteam needs it at launch to compose
the Claude-side name for Claude sessions), so choose it first with `kteam name`, then pass
BOTH `--teammate` and the plain `--name` in one `kteam start`.
...
1. **Get the task title** (natural Title Case, up to 5 words) if the user did not already say
   ...
   **PLAIN** title only — you never add a `[Teammate]` prefix, and you do **not** default it
   to the cwd basename. If the user gave a full title explicitly, use it as-is.
...
3. **Start with both flags in one shot** (run from the intended dir, or pass `--cwd`). Put
   ...
   callsign in `--name`. For a Claude session kteam composes the `[Hayden] Fix Transcript`
   Claude-side name itself:
...
Codex example (still pass the plain `--name` and `--teammate`; no RC, no composed Claude-side name):
...
One limitation to know (Claude sessions): rename only rewrites config, so it updates kteam's
**TASK column immediately**, but the **Claude-side session name only re-composes on the next
...
the next `kteam resume`. (A Codex session has no composed name, so only its TASK column moves.)
...
Report the **plain task title** AND — for a Claude session — the **Claude-side session
name** separately, since kteam composes the second from the first and they differ (one is
what the TASK column shows, the other is what claude.ai/code and the resume picker show):
...
The `Session` and `Remote` lines are Claude-only: Codex has no launch-time display-name flag
and no RC, so a Codex session has no `[Teammate] Task` name and no claude.ai link — report
just the plain Task for it.
